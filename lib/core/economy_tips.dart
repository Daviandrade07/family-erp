...
/// Preferência "Dica de economia ao abrir" — o pop-up é OPCIONAL: quem quiser
/// dicas as recebe; quem não quiser abre o app direto, sem interrupção.
class TipsController extends StateNotifier<bool> {
  TipsController() : super(true) {
    _load();
  }

  static const key = 'economy_tips_enabled';
...
final tipsEnabledProvider =
    StateNotifierProvider<TipsController, bool>((ref) => TipsController());

/// Exibe o pop-up de dica uma vez por sessão do app (se o usuário quiser).
class EconomyTipPopup {
  ...
  static Future<void> maybeShow(BuildContext context) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    if (!(prefs.getBool(TipsController.key) ?? true)) return;
