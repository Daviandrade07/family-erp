...
import '../../services/recurrence/recurrence_engine.dart' show RecurrenceFrequency;

export '../../services/recurrence/recurrence_engine.dart' show RecurrenceFrequency;
...
class Transaction {
  ...
  /// Regra recorrente que gerou esta transação (null = lançamento avulso).
  /// Liga a transação à sua recorrência para o mordomo comparar valores
  /// ao longo do tempo ("essa cobrança aumentou").
...
/// Regra recorrente (assinatura, salário, aluguel). A agenda é determinada por
/// [startedAt] + [frequency] + [intervalCount]; [nextRun]/[lastRun] são o
/// estado de geração. Matéria-prima do mordomo (Fase 3): [startedAt] permite
/// "repete há N meses"; transações geradas apontam de volta via recurring_id.
class RecurringTransaction {
  ...
  /// false (padrão): a IA SUGERE lançar, nunca posta sozinha (DNA do produto).
  final bool autoPost;
  final String? accountId;
  final String? cardId;
  ...
  factory RecurringTransaction.fromJson(Map<String, dynamic> j) =>
      RecurringTransaction(
        ...
        category: j['category'],
        description: j['description'],
        frequency: RecurrenceFrequency.values.byName(j['frequency']),
        intervalCount: (j['interval_count'] as num?)?.toInt() ?? 1,
        startedAt: _date(j['started_at'])!,
        nextRun: _date(j['next_run'])!,
        lastRun: _date(j['last_run']),
        endAt: _date(j['end_at']),
  ...
  Map<String, dynamic> toInsert() => {
        ...
        'started_at': _isoDate(startedAt),
        'next_run': _isoDate(nextRun),
        if (lastRun != null) 'last_run': _isoDate(lastRun!),
        if (endAt != null) 'end_at': _isoDate(endAt!),
        'active': active,
        'auto_post': autoPost,
...
class Bill {
  ...
  /// Conta vinculada (F-2): quando presente, pagar a bill gera uma despesa
  /// real nessa conta. Null = bill funciona apenas como lembrete.
  final String? accountId;
...
/// Sementes de categorias-padrão (viram linhas is_default=true no banco).
class Categories {
...
/// Categoria (padrão global ou customizada da família). Transações guardam o
/// NOME (texto), então arquivar/editar não afeta o histórico — só os seletores
/// de criação. [familyId] null = padrão global (lido por todos, não editável).
class Category {
  const Category({
    this.id,
    this.familyId,
  ...
  factory Category.fromJson(Map<String, dynamic> j) => Category(
        ...
        name: j['name'],
        type: TransactionType.values.byName(j['type']),
        colorToken: j['color_token'] ?? 'catGray',
        icon: j['icon'],
