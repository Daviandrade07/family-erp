  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          // A história vem antes dos números: o que aconteceu, por quê,
          // preciso me preocupar, próximo passo e o efeito no sonho.
          story.when(
            loading: () => const SizedBox(
                height: 220,
                child: LoadingSkeleton(itemCount: 1, itemHeight: 220)),
            error: (_, __) => const SizedBox.shrink(),
            data: (s) => _MonthStoryCard(story: s),
          ),
...
/// "Sua história do mês": as 5 respostas do motor de insights, num card único
/// e calmo. Os gráficos ficam abaixo, como prova — não como protagonista.
class _MonthStoryCard extends StatelessWidget {
  const _MonthStoryCard({required this.story});
  ...
  Widget build(BuildContext context) {
    ...
    final (Color moodColor, IconData moodIcon) = switch (story.mood) {
      StoryMood.tranquilo => (AppColors.neonGreen, Icons.check_circle_rounded),
      StoryMood.atencao => (AppColors.amber, Icons.info_rounded),
      StoryMood.cuidado => (AppColors.amber, Icons.error_outline_rounded),
    };
                ...
                children: [
                  ...
                  Text(label.toUpperCase(),
                      style: text.labelSmall?.copyWith(
                        ...
                        color:
                            color ?? text.bodySmall?.color?.withValues(alpha: 0.6),
                      )),
                ],
              ),
    ...
    return AppCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Sua história do mês',
              style: text.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
          const SizedBox(height: 14),
          row('O que aconteceu', story.whatHappened),
          row('Por quê', story.why),
          row('Preciso me preocupar?', story.shouldIWorry,
              color: moodColor, icon: moodIcon),
          row('Próximo passo', story.nextStep),
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 3),
                Text(story.dreamImpact, style: text.bodyMedium),
              ],
            ),
          ),
        ],
...
// Os KPIs do resumo vivem só em Finanças agora — Análises conta a história e
// mostra os gráficos, sem repetir o mesmo resumo (removido para não duplicar).
